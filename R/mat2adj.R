...
mat2adj.matrix <- function(x, infer.method='cor', use='all.obs', ...){
    ...
    ## Uncomment the following to restore
    ## if(method == 3L)
    ##     if (P > 1){
    ##         P <- 1
    ...
    if(!is.null(tmp)){
        ...
        myfun <- paste('Adj',METHODS[infer.method],sep='')
        tmp <- do.call(myfun, args=list(x=x, ...))
        return(tmp)
    }
}
